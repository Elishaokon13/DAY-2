...
import {
  ...
  TimelineDataPoint,
} from "@/app/api/earnings-timeline/route";
import {
  LineChart,
  Line,
  XAxis,
...
export function TimelineChart({
  ...
  const [timelineData, setTimelineData] =
    useState<EarningsTimelineResponse | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [period, setPeriod] = useState<string>(initialPeriod);
  const [chartType, setChartType] = useState<"earnings" | "cumulative">(
    "earnings",
  );
  ...
  useEffect(() => {
    async function fetchTimelineData() {
      ...
      try {
        console.log(
          `TimelineChart: Fetching timeline data for coin ${coinAddress}, period ${period}`,
        );
        ...
        if (!response.ok) {
          throw new Error(
            `Error fetching timeline data: ${response.statusText}`,
          );
        }
      ...
      return (
        <div className="bg-black p-3 border border-gray-700 text-white font-mono text-sm">
          ...
          <p className="text-xs text-gray-400">
            Volume:{" "}
            {formatCurrency(
              chartType === "earnings" ? data.volume : data.cumulativeVolume,
            )}
          </p>
        </div>
      );
    ...
    <Card className="p-6 bg-[#1a1e2e] border border-gray-700">
      ...
      <div className="mb-4">
        <p className="text-gray-400 font-mono text-sm mb-1">
          {timelineData.name} ({timelineData.symbol})
        </p>
      ...
      <div className="mt-4 text-gray-500 text-xs italic">
        <p>
          * Chart data is based on estimated trading patterns and creator
          earnings
        </p>
      </div>
    </Card>
