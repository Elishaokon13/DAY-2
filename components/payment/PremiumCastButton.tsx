...
export function PremiumCastButton({
  collageId,
  onPaymentComplete,
}: PremiumCastButtonProps) {
  const { context } = useMiniKit();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [transactionHash, setTransactionHash] = useState<string | null>(null);
  const [isPaid, setIsPaid] = useState(false);
  const [isWarpcastEnvironment, setIsWarpcastEnvironment] = useState<
    boolean | null
  >(null);
  const [walletReady, setWalletReady] = useState<boolean>(
    ...
    async function checkPaymentStatus() {
      try {
        const response = await fetch(
          `/api/payment/check-status?collageId=${collageId}`,
        );
  ...
  const handlePayment = async () => {
    ...
    if (!context?.user?.username) {
      setError(
        "Warplet wallet not connected. Please ensure you are using Warpcast.",
      );
      return;
    }
    ...
    try {
      ...
      if (!window.ethereum) {
        ...
        if (isWarpcastEnvironment) {
          setError(
            "Wallet connection is initializing. Please try again in a few seconds.",
          );
        } else {
          setError(
            "No Ethereum provider found. Please ensure you are using Warpcast.",
          );
        }
        setLoading(false);
        return;
